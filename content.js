...
function addBookmarkButtons() {
  ...
  messages.forEach((block) => {
    ...
    img.src = chrome.runtime.getURL(theme == "light"?'assets/bookmark-light.svg' : 'assets/bookmark-dark.svg');
...
function checkForPendingScroll() {
  chrome.storage.local.get({ pendingScroll: null }, (data) => {
    if (data.pendingScroll && data.pendingScroll.url === window.location.href) {
      console.log("Found pending scroll for message:", data.pendingScroll.messageId);
...
function findAndScrollToMessage(messageId, attempts = 0) {
  console.log('Looking for message:', messageId, 'attempt:', attempts);
  ...
  if (el) {
    ...
    if (attempts === 60) {
      console.log('Scrolling to top and trying again...');
      window.scrollTo(0, 0);
